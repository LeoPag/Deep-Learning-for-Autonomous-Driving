...
class MinMaxLogTensor:
    def __init__(self, modality, max, min):
        self.modality = modality
        self.max = max
        self.min = min
    ...
    def __call__(self, sample):
        if self.modality not in sample:
            return sample
        assert torch.is_tensor(sample[self.modality])
        max = torch.tensor(self.max, dtype=torch.float32).unsqueeze(-1).unsqueeze(-1)
        min = torch.tensor(self.min, dtype=torch.float32).unsqueeze(-1).unsqueeze(-1)
        sample[self.modality] = torch.log(sample[self.modality] - min + 1) / torch.log(max - min + 1)
        return sample
...
def get_transforms_mm(
    ...
    depth_meters_mean=None,
    depth_meters_stddev=None,
    depth_meters_max=None,
    depth_meters_min=None,
):
    return Compose([
        CropForPassableSidesPIL(crop_for_passable) if crop_for_passable > 0 else Identity(),
        RandomScaledTiltedWarpedCropPIL(
        ...
        ConvertToTensorsSimple(),
        ZeroMeanUnitVarianceTensor(MOD_RGB, rgb_mean, rgb_stddev) if rgb_mean is not None else Identity(),
        MinMaxLogTensor(MOD_DEPTH, depth_meters_max, depth_meters_min) if depth_meters_mean is not None else Identity(),
    ])
